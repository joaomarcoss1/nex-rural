...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data, error } = await admin
      ...
      .update({ portal_visible: true, status: "Liberado no portal", released_to_portal_at: new Date().toISOString(), updated_at: new Date().toISOString() })
      .eq("id", id)
    ...
    await admin.from("audit_logs").insert({
      ...
      action: "generated_document_release",
      entity: "generated_documents",
      entity_id: id,
      record_table: "generated_documents",
      record_id: id,
      new_value: { portal_visible: true, operated_as_admin_master: actor.role === "admin_master_global" },
      user_agent: request.headers.get("user-agent")
    });
    return NextResponse.json({ ok: true, generated_document: data });
  } catch (err) {
    return NextResponse.json({ error: err instanceof Error ? err.message : "Falha ao liberar documento." }, { status: 400 });
  }
}
